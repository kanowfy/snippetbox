...
func (m *SnippetModel) Insert(title, content, expires string) (int, error) {
	...
	// sql.Result contain LastInsertId() and RowsAffected() method
	...
	// id return is of type int64 so we need to convert first 
	return int(id), nil
}
...
func (m *SnippetModel) Latest() ([]*models.Snippet, error) {
	stmt := `SELECT id, title, content, created, expires FROM snippets WHERE
	expires > UTC_TIMESTAMP() ORDER BY created DESC LIMIT 5`
